...
void vector_create(vector* v, unsigned int vectorSize) {
   ...
   v->elements = (Song*)malloc(vectorSize * sizeof(Song));
   v->size = vectorSize;
   for (i = 0; i < v->size; ++i) {
...
void vector_resize(vector* v, unsigned int vectorSize) {
   ...
   oldSize = v->size;
   v->elements = (Song*)realloc(v->elements, vectorSize * sizeof(Song));
   v->size = vectorSize;
...
void InsertPlaylistNodeAfter(vector* v, unsigned int index, Song value) {
   int i;
...
void vector_push_back(vector* v, Song value) {
   InsertPlaylistNodeAfter(v, v->size, value);
}
